...
<head>
  ...
  <script src="//dapi.kakao.com/v2/maps/sdk.js?appkey=f3a5008edad1780fc2ddff81474f335f&libraries=clusterer,services"></script>
...
function renderMarketItem(name, addr){
  return ''
    ...
    +     ' data-market-name="' + esc(name) + '"'
    +     ' data-market-addr="' + esc(addr) + '">'
...
$(document).ready(function () {
  ...
  $("#searchBtn").click(function () {
    ...
    if (!sido) { alert("시도를 선택하세요."); return; }
    ...
    $.ajax({
      ...
      dataType: "json",
      data: { sido: sido, sigungu: sigungu, marketName: marketName },
      success: function (data) {
        ...
        if (!data || data.length === 0) {
          ...
          html = data.map(function(item){
            var name = (item["시장명"] || "").trim() || "시장명 없음";
            var addr = ((item["지번주소"] || "").trim())
                    || ((item["도로명주소"] || "").trim())
                    || "주소 정보 없음";
            return renderMarketItem(name, addr); // ☜ JSP와 동일 마크업
          }).join("");
        }

        $("#initialSeoulList").hide();       // 초기 리스트 숨김
        $("#resultArea").html(html).show();  // 결과 렌더
      },
  ...
  $(document).off('click', '.js-market-link').on('click', '.js-market-link', function (e) {
    e.preventDefault();
    const $a = $(this);
    const name = $a.data('name') || ($a.text() || '').trim();
    const addr = $a.data('addr') || ($a.closest('.market-item').find('.market-address').text() || '').trim();
...
function viewMarketDetail(marketName, address) {
  console.groupCollapsed("%cviewMarketDetail ▶", "color:#6f42c1;font-weight:bold;");
  ...
  $.ajax({
    ...
    type: "GET",
    dataType: "json",
    data: { marketName: marketName, addr: address || "" },
    success: function (data, status, xhr) {
      console.groupCollapsed("%cgetMarketCoords ✓", "color:#198754;font-weight:bold;");
      console.log("HTTP", xhr.status, xhr.statusText);
      console.log("Response:", data);
      console.groupEnd();

      var lat = (data && data.latitude != null) ? Number(data.latitude) : NaN;
      var lng = (data && data.longitude != null) ? Number(data.longitude) : NaN;
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        if (typeof window.debugNoCoords === 'function') {
          window.debugNoCoords({ data: data, marketName: marketName, address: address, xhr: xhr });
        }
        alert("해당 시장의 위치 정보를 찾을 수 없습니다.");
      }
    },
    error: function (xhr) {
      console.groupCollapsed("%cgetMarketCoords ✗", "color:#dc3545;font-weight:bold;");
      console.log("HTTP", xhr.status, xhr.statusText);
      ...
      <div class="market-item"
        ...
        <div class="market-name">
          ...
          <a href="#" class="js-market-link"
             ...
             data-addr="<c:out value='${item["지번주소"]}' />">
